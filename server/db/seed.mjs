...
async function createSchema(client) {
  ...
  for (const t of ['concert_likes', 'concert_songlist', 'concert_images', 'concert_tags', 'guestbook_reply']) {
    await client.execute(`DROP TABLE IF EXISTS ${t};`);
  }
  // 再删主表· then drop parent tables
  for (const t of ['concerts', 'cities', 'wishes', 'guestbook']) {
    await client.execute(`DROP TABLE IF EXISTS ${t};`);
  }
...
async function main() {
  ...
  const tables = ['concerts', 'concert_likes', 'concert_tags', 'concert_images', 'concert_songlist', 'cities', 'wishes', 'guestbook', 'guestbook_reply'];
  const counts = {};
